...
| Identity | Operations key off the internal `invID` / `awID` (not the 發票號碼). The provider resolves the invID from the invoice number via `invoice/list`, or you can pass `providerOptions.invID` (the issue result's `raw.id`) to skip the lookup. |
| Amounts | The platform computes tax (`trCode` 0 = 5%); `prodList[].sales` is the unit price, `incTax` follows `priceMode`. |

## Operations

```ts
// 開立 — B2C (member carrier), B2B (統編), donation, mobile barcode, mixed tax…
...
- **Mixed tax**: set per-item `taxType` (應稅 / 零稅率 / 免稅).

## Capabilities

`ISSUE` · `VOID` · `ALLOWANCE` · `VOID_ALLOWANCE` · `QUERY` · `B2B` · `MIXED_TAX`.

Not declared: `FOREIGN_CURRENCY` (true 境外電商 / carrierType 20 needs a
境外電商-type account — a normal account returns `1052`), `SCHEDULED_ISSUE`,
`CARRIER_VALIDATION`.
...
| `stID` | | partner store id (`x-deva-stid`) for 合作廠商 access |
| `mode` | | `"TEST"` (default, `tryapi`) or `"PRODUCTION"` (`api`) |
| `validatePayload` | | validate the issue payload locally (default `true`) |

Notes: 字軌 配號 (acquiring invoice number ranges) is **backend-only** — the API
can only manage existing 字軌. Live tests run with `EZRECEIPT_LIVE=1` against a
